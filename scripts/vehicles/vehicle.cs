...
// Base Seed
$BaseSeed::DeployTimeMS = 5000;


function BaseSeed::onAdd(%this, %obj)
{
   Parent::onAdd(%this, %obj);

   %bot = AIConnect("Base Seed Pilot", 1);
   %bot.schedule(256, "setControlObject", %obj);
   %bot.setPilotDestination(%obj.getPosition());
   schedule(128,0,"eval",%bot.player @ ".delete();");

   setTargetSensorGroup(%obj.getTarget(), %obj.team);
}

function BaseSeed::deployDone(%this, %obj, %targetPosition)
{
    // We're done, do a direct transform to target for my own sanity.
    if (!%obj.isDeployed)
    {
        // Here we restore the vehicle, however.
        %obj.hide(false);
        %obj.setFrozenState(false);
        %obj.startFade(0, 0, 0);

        %obj.setTransform(%targetPosition);
        %obj.deployed.setTransform("0 0 0");
        %obj.undeployed.setTransform("0 0 0");
    }
    else
    {
        // Force roll to zero, also for my own sanity.
        %targetTransform = %obj.undeployed.getTransform();
        %targetTransform = %targetPosition SPC getWords(%targetTransform, 3, 6);
        %targetTransform = setWord(%targetTransform, 4, "0");

        %obj.undeployed.setTransform("0 0 0");
        %obj.deployed.setTransform(%targetTransform);

        // Now that we're deployed, we want to make sure no derps got
        // stuck inside of our vessel.
        %size = %obj.deployed.getWorldBoxSize();
        %maxDist = getWordMax(%size);

        // Random variances in distance
        %randomMin = %maxDist + 10;
        %randomMax = %maxDist + 20;

        InitContainerRadiusSearch(%obj.getWorldBoxCenter(), %maxDist, $TypeMasks::VehicleObjectType | $TypeMasks::PlayerObjectType);
        while ((%current = ContainerSearchNext()) != 0)
        {
            // Skip ourselves or if we encounter something that's mounted
            if (%current == %obj || isObject(%current.getObjectMount()) || !%current.worldBoxIntersects(%obj.deployed))
                continue;

            // We found a trapped victim, let's free them.
            %targetWorldSize = %current.getWorldBoxSize();
            %newPosition = vectorRandomOn(%randomMin, %randomMax, %obj.deployed.getPosition(), -1);
            %newPosition = setWord(%newPosition, 2, getWord(%newPosition, 2) + getWord(%targetWorldSize, 2));
...
function BaseSeed::deploySequence(%this, %obj, %deployed)
{
    // Grab all current information
    %currentTransform = %obj.deployed.getTransform();
    %currentPosition = getWords(%currentTransform, 0, 2);
    %currentHeight = getWord(%currentPosition, 2);

    // If we're deploying, we're using the ground as a reference point
    if (%obj.isDeployed)
        %targetHeight = getTerrainHeight(%currentPosition);
    else // Otherwise, it's wherever our stabilizer min is
        %targetHeight = getTerrainHeight(%currentPosition) + %obj.getDatablock().stabLenMin;

    %targetPosition = getWords(%currentPosition, 0, 1) SPC %targetHeight;

    // Are we close enough to our destination?
    if (vectorDist(%currentPosition, %targetPosition) <= 1)
    {
        BaseSeed.deployDone(%obj, %targetPosition);
        return;
    }

    // Nope, let's move a little towards it.
    %heightDelta = (%targetHeight - %currentHeight) / 32;
    %currentHeight += %heightDelta;

    // While we're at it, kill rolls incrementally
    %currentRoll = getWord(%currentTransform, 3);
    %currentRoll += -(%currentRoll / 32);

    // Update our transformations and reloop
    %currentTransform = setWord(%currentTransform, 2, %currentHeight);
    %currentTransform = setWord(%currentTransform, 3, %currentRoll);

    %obj.deployed.setTransform(%currentTransform);
    %obj.undeployed.setTransform(%currentTransform);
    %obj.deploySequence = BaseSeed.schedule(32, "deploySequence", %obj);
}

function BaseSeed::deploy(%this, %obj)
{
    if (isEventPending(%obj.deploySequence))
        cancel(%obj.deploySequence);

    // Hide the actual vehicle
    %obj.startFade(1, 0, 1);
    %obj.setFrozenState(true);
    %obj.hide(true);

    %obj.isDeployed = !%obj.isDeployed;

    // If it doesn't have a deployed/undeployed state already, make them.
    if (!isObject(%obj.deployed))
    {
        %obj.deployed = new StaticShape()
        {
            datablock = DeployedBaseSeed;
            vehicle = %obj;
        };

        %obj.deployed.setTarget(%obj.getTarget());
    }

    if (!isObject(%obj.undeployed))
    {
        %obj.undeployed = new StaticShape()
        {
            datablock = UndeployedBaseSeed;
            vehicle = %obj;
        };

        %obj.undeployed.setTarget(%obj.getTarget());
    }

    if (%obj.isDeployed)
    {
        %obj.deployed.setTransform(%obj.getTransform());
        %obj.undeployed.setTransform(%obj.getTransform());
    }

    // Fade both portions in
    %obj.deployed.startFade($BaseSeed::DeployTimeMS, 0, !%obj.isDeployed);
    %obj.undeployed.startFade($BaseSeed::DeployTimeMS, 0, %obj.isDeployed);

    // Slowly push both to the ground as they deployed
    BaseSeed.deploySequence(%obj);
}
